...
void* upo_ht_sepchain_put(upo_ht_sepchain_t ht, void *key, void *value)
{
    ...
    void *old_value = NULL;
    if(ht->size/ht->capacity >= 0.5) ht->capacity *=2;
...
void* upo_ht_sepchain_get(const upo_ht_sepchain_t ht, const void *key)
{
    size_t h = ht->key_hash(key, ht->capacity);
    upo_ht_sepchain_list_node_t* n = ht->slots[h].head;
    while(n != NULL && ht->key_cmp(key, n->key) != 0) n = n->next;
...
void upo_ht_sepchain_delete(upo_ht_sepchain_t ht, const void *key, int destroy_data)
{
    ...
    if(n != NULL){
        if(p == NULL) ht->slots[h].head = n->next;
        else p->next = n->next;
        if(destroy_data){
            free(n->key);
            free(n->value);
...
void* upo_ht_linprob_put(upo_ht_linprob_t ht, void *key, void *value)
{
    ...
    int found = 0;
    size_t h_tomb = 0;
    while((ht->slots[h].key != NULL && key != ht->slots[h].key) || ht->slots[h].tombstone){
        if(ht->slots[h].tombstone){
            found = 1;
            h_tomb = h;
        }
        h = (h + 1) % ht->capacity;
...
int upo_ht_linprob_contains(const upo_ht_linprob_t ht, const void *key)
{
    ...
    if(ht->slots[h].key != NULL) return 1;
    else return 0;
}
...
{
    if(ht == NULL || upo_ht_sepchain_is_empty(ht)) return NULL;

    upo_ht_key_list_t lis = NULL;
    upo_ht_key_list_t* tail = &lis;
...
{
    if(ht == NULL || upo_ht_linprob_is_empty(ht)) return NULL;
    ...
    for(size_t h = 0; h < ht->capacity; h++){
        ...
        if(n->key != NULL && !n->tombstone){        //n non è mai NULL in linprob (se esiste ht esiste n) ma i suoi elementi potrebbero esserlo
            upo_ht_key_list_node_t* node = malloc(sizeof(upo_ht_key_list_node_t));
...
{
    if(ht == NULL || upo_ht_linprob_is_empty(ht)) return ;

    for(size_t h = 0; h < ht->capacity; h++){

        upo_ht_linprob_slot_t* n = &ht->slots[h];
        if(n->key != NULL && !n->tombstone)visit(n->key, n->value, visit_context);
        }
}
